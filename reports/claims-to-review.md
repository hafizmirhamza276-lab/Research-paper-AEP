...
**Status: RESOLVED 2026-09-25, all three.** Raised 2026-09-25 immediately
after entry 10 was closed, and applied the same day by author instruction.
None of the three was a wrong number; each was an omission or an imprecision.
What was changed is at § *How it was resolved* below.
...
### How it was resolved, 2026-09-25

All three applied by author instruction. The manuscript now says what the
figures show.

**12.1 — `06-evaluation.tex`, the caption of `fig:bycrashpoint`.** The
exclusion is now stated in the caption, in \cref{tab:outcomes}'s own words so
the two agree rather than paraphrase each other:

> The systems do not all plot the same crash points, for the reasons
> \cref{tab:outcomes} gives: B4 and B4b have no bar at
> `after_barrier_before_dispatch`, those cells excluded because the harness
> delivered that kill inside the socket wait rather than before dispatch, and
> B0 to B2 have none there for that reason and none at
> `after_intent_before_barrier`, which cannot occur in a system that writes no
> intent. AEP-full and B3 give the two positions different kills and plot all
> six, at zero throughout.

The clauses *"excluded because the harness delivered that kill inside the
socket wait rather than before dispatch"* and *"cannot occur in a system that
writes no intent"* are lifted from \cref{tab:outcomes}'s caption unchanged.

**12.2 — `06-evaluation.tex`, the sentence formerly at :288.**

> **Before:** The figure adds no number this section does not state
>
> **After:** Every rate the figure plots lies within the values this section
> states

This is the minimal wording 12.2 proposed. It is true of all nine plotted
rates, including the two the old sentence was wrong about: B4's 0.4778 lies
inside its stated per-class 0.4467/0.5067/0.4800, and the declared-ambiguity
bars at 0.3574 (AEP-full) and 0.3611 (B3) lie inside their stated per-class
0.0000/0.3500/0.7222 and 0.0000/0.3667/0.7167. The rest of the sentence, on
Wilson versus cluster-aware intervals, is unchanged.

**12.3 — the two provenance comments.** Both now name `crashed`, and both
record the relabelling that made `(session-3)` misleading, so the next person
to regenerate does not filter on a label the generator has already rewritten:

```
% Generated by experiments/analyze.py write_figures(), regime "crashed".
% scripts/rebuild_paper_figures.py relabels the legacy "(session-3)" rows to
% "crashed" before plotting, so "(session-3)" is not the regime this figure is
% filtered to even though it is the label in the tracked CSV.
```

in `06-evaluation.tex` beside `fig:bycrashpoint`, and the same note beside
`supp:fig:trade` in `supplementary.tex`, where the existing *"NOT from
table-1.csv, which pools regimes"* is kept. `01-introduction.tex:104` was left
alone: it already says the two labels are one regime, which is correct.

### Evidence

All four PDFs rebuilt in dependency order — `main-anon`, `supplementary`,
`supplementary-anon`, then `main`, which is last because it is the only build
that runs `check_paper_numbers.py` and that check tests the other three for
staleness. Page counts unchanged at 25 and 7, so the added caption text cost
no page.

Both edits confirmed present in the rendered `main.pdf` by `pdftotext`, not
only in the source. Every gate re-run after the change:

| gate | result |
|---|---|
| `prove_anonymous_gate.sh` | all four checks observed failing on their own leak, tree restored clean |
| `check_paper_numbers` | 44 passed, 0 failed |
| `check_no_repo_paths` | 4 builds clean |
| `check_american_spelling` | 4 builds clean |
| `check_no_ai_language` | 4 builds clean |
| `check_prereg_order` | 36 cells, 31 ok, 5 exempt, 0 failing |
| `render_arxiv_abstract --check` | 3 ok, 0 failed |
