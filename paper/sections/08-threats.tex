...
is the semantics, and the ablation says which half delivers which, including
that the expensive half is optional.
...
\paragraph{A failure to reject is not a null result here, and for two of them
that is a theorem} Two comparisons in this paper fail to exclude zero for
...
$\SignFloorFour{}$. \textbf{No effect size, however large, could have produced a
rejection at $\alpha=0.05$ from these designs.} Six sessions is the smallest at
which the test becomes capable of rejecting at all
($2/2^{6} = \SignFloorSix{}$). So what we report is not that we looked for an
effect and failed to find one; it is that these designs could not have found
one, whatever was there. Neither result should be read as evidence that the
effect is absent, and neither is offered as one.
