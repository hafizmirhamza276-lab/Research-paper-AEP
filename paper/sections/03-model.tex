...
This section condenses \texttt{docs/22-formal-model.md} in the
...
\item[\textbf{F3}] \textbf{Redis restart with AOF replay.} Up to roughly one
      ...
      considerably. The set of faults that can actually realise that loss is
      narrower than the documentation's wording suggests, and a process kill
      is not among them; a host-level write loss is, and realises the loss
      every time.
...
its own effects; loss of the Redis host once the fsync acknowledgement has been
