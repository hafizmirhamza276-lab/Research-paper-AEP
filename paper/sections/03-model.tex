...
\item[\textbf{F3}] \textbf{Redis restart with AOF replay.} Up to roughly one
      ...
      \texttt{appendfsync everysec}~\cite{redis-persistence}. \Cref{sec:eval-rq2}
      reports a measurement that sharpens this considerably: which
      \emph{faults} can actually realise that loss is narrower than the
      documentation's wording suggests.
...
\midrule
\texttt{before\_intent\_write}            & no record, no effect \\
\texttt{after\_intent\_before\_barrier}   & record in memory, maybe not on disk; no effect \\
\texttt{after\_barrier\_before\_dispatch} & record durable; \textbf{no effect} \\
\texttt{mid\_dispatch}                    & record durable; effect \textbf{likely} \\
\texttt{after\_response\_before\_resolution} & record durable; effect known to the dead process only \\
\texttt{after\_resolution\_before\_barrier}  & outcome written, not yet acknowledged \\
\bottomrule
\end{tabular}
\end{table}
