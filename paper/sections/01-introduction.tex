...
\item An \textbf{\undetdup{}}. The recovery process re-sends. If the first
      ...
      So do a lease, a fenced compare-and-swap and a durable event-sourced
      history, as \cref{sec:evaluation} shows, because none of them writes a
...
\item A \textbf{\lostfx{}}. The recovery process does not re-send and records
      ...
      produces this outcome. It is quieter than a duplicate and no less
      serious: a refund that the ledger says did not happen becomes a
      reconciliation problem that surfaces in the accounts weeks later.
...
This impossibility is structural. Deciding whether a remote effect occurred,
...
can choose is where the uncertainty is allowed to surface. AEP places it in a
durable state that an operator can see, and keeps it out of the accounts.
...
\item[\textbf{C2}] \textbf{A protocol and an implementation} in which a
      ...
      assumption, the fsync acknowledgement returns an opaque, non-copyable,
...
\item[\textbf{C3}] \textbf{An evaluation under real process kills} across six
      ...
      none crashed, Redis hard-killed before the acknowledgement, Redis killed
...
\item[\textbf{C4}] \textbf{A decomposition of the mechanism, by ablation,
      ...
      normally presented as one mechanism delivering one guarantee, and we
      find that it is two. The pre-dispatch record together with no re-entry
