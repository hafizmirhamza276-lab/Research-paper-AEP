...
**Character count of the abstract block above: 3 055** (measured, not estimated).
...
```
Autonomous agents invoke legacy APIs that are
...
as one. Detection -- no undetected duplicate and no lost effect, with residual
declared ambiguity set by endpoint capability -- comes from a pre-dispatch
record plus no re-entry into dispatch. In the crashed regime, B3 and AEP-full
each record zero such silent failures over 540 executions; their ambiguity
difference is 0.37 percentage points with a stratified run-cluster 90% interval
of [-1.11, 2.04]. Prevention is what the barrier buys against a different fault.
In one no-readback capability class at one pre-acknowledgement Redis-kill point
on one host, it withholds effects the ablation puts on the wire (10/30 versus
28/30, Fisher p = 1.9e-6). The prevention scope is narrow, and the barrier's
cost remains a deployment choice.
```

**Character count: 1 905.** arXiv counts characters, not words; re-measure with
