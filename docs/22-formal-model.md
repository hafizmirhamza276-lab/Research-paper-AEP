...
(`PAPER_ROADMAP.md:53-69`) after the four correctness fixes landed.
...
2. **Where the code does not enforce a property, this document says so.** The
   ...
   `docs/06-phase2-design.md` but *not enforced* in `src/core/`.
...
|---|---|
| Response classes were a test-only construct (§1.5) | **Closed.** The contract is `src/core/connector_contract.py`; `tests/mock_connector.py:24-27` imports it. |
| Recovery had no fault isolation (§5.3, assumption A2) | **Closed.** `src/core/intent_recovery.py:192-261`, `:263-319`. |
...
|---|---|---|
| **Runner** (worker) | `WriteAheadRunner` — `src/core/intent_workflow.py:139` | Performs at most one external mutation per invocation of `execute` (`src/core/intent_workflow.py:363`). |
| **Recovery service** (resolver) | `IntentRecoveryService` — `src/core/intent_recovery.py:96` | Read-only with respect to the external system; writes only Redis state. |
| **Connector** | `ExternalMutationConnector` — `src/core/intent_workflow.py:54`; reconciliation side `ReconciliationConnector` — `src/core/connector_contract.py:101` | The only component that touches the external API. |
| **Store** | Single Redis instance, via `IntentLedgerStore` (`src/core/intents.py:706`) and `RedisStorageAdapter` (`src/core/storage.py:393`) | |
| **Operator** | *Not implemented as a component.* | The only operator surface is `transition_intent` (`src/core/intents.py:874`) with an `actor` string. See §5.3. |
...
(`src/core/intent_recovery.py:263`) is a library coroutine that some external
...
|---|---|---|
| `aep:state:{execution_id}` | the two state Lua scripts only | `src/core/intents.py:601`, `src/core/storage.py:266` |
| `aep:lock:{execution_id}` | `SET NX EX` / token-checked Lua `DEL`/`PEXPIRE` | `src/core/locks.py:149-152`, `:35-52`, `:54-70` |
| `aep:dispatch-auth:{execution_id}:{intent_id}` | the dispatch-authorization Lua only | `src/core/intents.py:673`, key builder `:1076` |
| `aep:vault:{locator}` | the evaluation vault (EVALUATION mode only) | `src/core/request_vault.py:372` |
| `aep:poison:{execution_id}:{epoch_ms}` | best-effort quarantine writer | `src/core/storage.py:784-822` |

**Authoritative object.** `aep:state:{execution_id}` holds one JSON document
validated as `Phase2ExecutionState` (`src/core/intents.py:339`), extending
`AEPExecutionState` (`src/core/storage.py:82`) with a typed
`intent_ledger: dict[str, IntentRecord]`. Serialisation is deterministic
(`src/core/state_codec.py:118-138`); decoding rejects duplicate JSON member
names (`src/core/state_codec.py:67-75`). A Lua re-implementation of the same
strict UTF-8 + duplicate-member check is prefixed to every authoritative script
(`src/core/state_codec.py:151-421`, `:425-428`).

**Version.** `version: int` is the fencing counter, bounded to 2^53−1 because
Redis Lua numbers are IEEE-754 doubles (`src/core/storage.py:74-76`). It is
deliberately distinct from the lock ownership token, a random
`secrets.token_urlsafe(32)` (`src/core/locks.py:150`, documented `:126-129`).

**State-key lifetime.** Every authoritative write sets an expiry
(`src/core/intents.py:601`; `src/core/storage.py:266`). The Lua rejects a TTL
below 31 days whenever the resulting ledger contains an `ABOUT_TO_FIRE`,
`FIRED_UNCONFIRMED`, **or `PERMANENTLY_AMBIGUOUS`** record
(`src/core/intents.py:592-599`, floor constant `:49`). The ledger is
...
- **Connection pinning** is required for CAS-then-durability, because
  ...
  `IntentLedgerStore.pinned_connection` (`src/core/intents.py:729`), used at
  `src/core/intent_workflow.py:424`, `:511`, `:619` and
  `src/core/intent_recovery.py:379`, `:539`.
- Transport faults become typed, non-silent failures:
  `LockAcquisitionError` (`src/core/locks.py:153-157`),
  `StorageOperationError` (`src/core/storage.py:515-516`),
  `IntentStateError` (`src/core/intents.py:1022-1025`),
  `WriteAheadWorkflowError` (`src/core/intent_workflow.py:335-338`).
...
|---|---|---|
| Redis server `TIME` | `prepared_at`, `reconcile_after`, `next_check_at`, audit times, recovery eligibility | `src/core/intents.py:735`; recovery at `src/core/intent_recovery.py:202`, `:367` |
| Redis key expiry / `PTTL` | lease liveness, pre-dispatch TTL floor, dispatch-authorization lifetime | `src/core/locks.py:152`; `src/core/intents.py:618`, `:673` |
| Process-local `monotonic` | heartbeat interval, deadline watchdog, dispatch-capability expiry, recovery pass SLO | `src/core/locks.py:350-357`, `:406-420`; `src/core/request_binding.py:1191`; `src/core/intent_recovery.py:288` |
...
- `T_client <= T_lock − Buffer_Margin`, `Buffer_Margin >= 15s`
  ...
  `ConnectorPolicy.__post_init__` (`src/core/intent_workflow.py:97-104`) and in
  `DistributedLockManager.lease` (`src/core/locks.py:292-321`). It is a timing
...
(`src/core/intent_workflow.py:84-90`, `:119-124`). Any other value, and any
exception, is coerced to ambiguity (`src/core/intent_workflow.py:591-616`).
...
|---|---|---|---|
| `AUTHORITATIVE_READBACK` | `APPLIED`, `NOT_APPLIED`, `UNKNOWN`, `CONFLICT` | `docs/06-phase2-design.md:351` | `src/core/connector_contract.py:36-45`, table `:78-99` |
...
now defined in `src/core/connector_contract.py` and the test harness imports
them (`tests/mock_connector.py:24-27`). Three consequences:

- The capability is resolved by `declared_capability`
  (`src/core/connector_contract.py:118-140`), which **fails closed** on a
...
- Classification is total and explicit per class in `classify_readback`
  (`src/core/connector_contract.py:171-240`). `POSITIVE_ONLY_READBACK` +
  `NOT_APPLIED` is a *named* contract violation
  (`src/core/connector_contract.py:220`), not a fall-through.
- Recovery consumes the typed contract, not string literals
  (`src/core/intent_recovery.py:404`, `:424`, `:469`); a regression test
...
(`src/core/connector_contract.py:47-54`). Unparseable or absent evidence
degrades to `UNKNOWN` (`src/core/connector_contract.py:142-169`) so it consumes
...
`ReconciliationContext` (`src/core/request_binding.py:1065-1095`) built at
`src/core/intent_recovery.py:441-453`: identifiers, a redacted target, and the
fingerprint — no request material, no capability object.

### 1.6 Dispatch authority

A mutation call requires a `VerifiedDispatch`
(`src/core/request_binding.py:1109-1131`), which cannot be constructed
(`:1115-1116`), subclassed (`:1118-1119`), or copied (`:1121-1125`); is minted
only by the wrapper over `RequestBindingService.verify`
(`src/core/request_binding.py:1489-1503`); and is single-use
...
prepared one (`src/core/intent_workflow.py:530-554`).

**Phase 1B adds a second, Redis-visible gate** (§3.2 B).

### 1.7 Composition modes

`DispatchMode` (`src/core/intent_workflow.py:39-52`) is checked by
`validate_startup` (`src/core/intent_workflow.py:226-309`). An unspecified mode
is `PRODUCTION`; a caller that passed `allow_test_dispatch` without naming a
mode is placed in `TEST` by that act (`src/core/intent_workflow.py:162-166`).
...
(`src/core/request_vault.py:321`): AES-GCM, create-once
...
(`src/core/request_vault.py:325-339`).
...
(`src/core/intent_workflow.py:204-210`, `src/core/intent_recovery.py:136-143`).
...
1. **Intent and resolution writes get an explicit fsync barrier.**
   ...
   least one *local* fsync is reported (`src/core/durability.py:318`, `:332`).
2. **Lock keys and the dispatch-authorization key are not barriered.**
   `src/core/locks.py:119-240` and `src/core/intents.py:673` are ordinary
...
- Delay / no response: `TIMEOUT_NO_RESPONSE`,
  ...
  ambiguous at `src/core/intent_workflow.py:607-616`.
...
- A late effect cannot be recalled (`docs/06-phase2-design.md:266`); recovery
  waits until `reconcile_after` (`src/core/intents.py:836-841`) before drawing
...
1. **Pre-dispatch preflight** atomically re-checks lock token
   (`src/core/intents.py:617`), `PTTL >= T_client + Buffer_Margin`
   (`src/core/intents.py:618-619`), version (`:624`), status (`:626`), binding
...
2. **Cancellation of the protected task** by the heartbeat on ownership loss
   (`src/core/locks.py:394-404`), the deadline watchdog (`:406-420`), and the
...
- **Committed write** — a state write for which the Lua CAS returned `1`
  (`src/core/intents.py:602`, `src/core/storage.py:267`).
- **Durable write** — a committed write for which `WAITAOF` reported at least
  one local AOF fsync on the *same connection* (`src/core/durability.py:332`).
- **Dispatch** — invocation of `connector.mutate`
  (`src/core/intent_workflow.py:574`).
...
- **Resolved** — status is `FIRED_CONFIRMED`, `FAILED_CONFIRMED`, or
  ...
  `{ABOUT_TO_FIRE, FIRED_UNCONFIRMED}` (`src/core/intents.py:61-63`).
- **Roadmap vocabulary** (`PAPER_ROADMAP.md:36`): CONFIRMED =
  `FIRED_CONFIRMED`; REFUTED = `FAILED_CONFIRMED`; PERMANENTLY_AMBIGUOUS =
  `PERMANENTLY_AMBIGUOUS` (`src/core/intents.py:53-59`). The same three plus
  `RETRY` appear as `ReconciliationOutcome`
  (`src/core/connector_contract.py:56-68`), mapped to statuses at
  `src/core/intent_recovery.py:43-51`.
...
|---|---|---|
| Single atomic invocation | body `src/core/intents.py:372-603`, assembled `:605`, registered `:722`, invoked from `commit_transition` `:972` | `src/core/storage.py:159-268`, assembled `:270`, registered `:421`, invoked `:505-514` |
| (a) live-token check | `src/core/intents.py:385` | `src/core/storage.py:202` |
| (b) exact expected-version CAS | `src/core/intents.py:399-400` | `src/core/storage.py:244` (creation `:255-257`) |
| The write itself | `src/core/intents.py:601` | `src/core/storage.py:266` |
| Typed rejection | `src/core/intents.py:1026-1073` | `src/core/storage.py:518-556` |
...
`schema_version` (`src/core/intents.py:402-403`); the Phase 2 marker
...
either (`src/core/storage.py:250-252`, `:262-265`, mapped `:547-552`).
...
- **R1-2 — Expiry, not deletion, ends the guarantee.** Each write resets the
  key TTL (`src/core/intents.py:601`). After expiry the record is gone and any
  writer may create version 1 again (`src/core/storage.py:255-257`). Phase 1B
  extended the 31-day floor to cover `PERMANENTLY_AMBIGUOUS`
  (`src/core/intents.py:592-599`), so an escalated record can no longer be
...
- **R1-3 — AOF rewind can un-fence. CONFIRMED BY PROBE.** P1 is a statement
  ...
  *Evidence:* `tests/test_residual_probes.py:46-121` performs a deterministic
  ...
  `src/core/locks.py`). **No local fix exists**: the fix is consensus/HA, an
...
- **R1-5 — The token is a bearer credential.** Nothing proves the caller
  obtained it from `acquire_lock` (`src/core/intents.py:385`).
- **R1-6 — Raw-state classification precedes the token check**
  (`src/core/intents.py:373-383` then `:385`). A caller without a valid token
  can learn whether stored state is corrupt and can cause a best-effort
  `aep:poison:*` write (`src/core/storage.py:671-691`). No state write results.
...
(`src/core/intent_workflow.py:363`):
...
1. `confirm_durable_ack` (`src/core/durability.py:161-176`) runs the barrier
   and mints a `DurabilityAck` **only** when it returned `True`.
2. `DurabilityAck` (`src/core/durability.py:75-102`) cannot be constructed
   (`:87-90`), subclassed (`:92-93`), or copied (`:95-99`); it is single-use
   and scope-bound via an HMAC provenance registry
   (`src/core/durability.py:105-155`). The scope is
   `execution_id:intent_id:prepared_state_version`
   (`src/core/durability.py:62-72`), so an ack for one attempt cannot authorise
   another.
3. `IntentLedgerStore.authorize_dispatch`
   (`src/core/intents.py:1081-1162`) **consumes** the ack first (`:1110-1115`),
   then runs `_DISPATCH_AUTHORIZATION_SCRIPT`
   (`src/core/intents.py:649-675`), which re-checks lease, version, status and
   binding digest and writes `aep:dispatch-auth:{exec}:{intent}` (`:673`).
4. The preflight Lua requires that exact value
   (`src/core/intents.py:639`); an absent authorization defaults to a value
   that cannot match (`src/core/intents.py:1186`), so it fails closed with
   `DispatchAuthorizationError` (`src/core/intents.py:136-137`, raised
   `:1223-1226`).
...
exception, →`FIRED_UNCONFIRMED` (`src/core/intent_workflow.py:578-616`). The
...
(`src/core/intent_recovery.py:145-151`), under a freshly acquired lease
(`:362-364`) and after re-validating status, version and eligibility
(`:366-377`):

| Observation | Target | Citation |
|---|---|---|
| capability undeclared / unrecognised | `PERMANENTLY_AMBIGUOUS`, **no query** | `src/core/intent_recovery.py:403-422` |
| `NO_READBACK` | `PERMANENTLY_AMBIGUOUS`, no query | `:424-437` |
| `APPLIED` (either querying class) | `FIRED_CONFIRMED` | `src/core/connector_contract.py:196-210` |
...
| `UNKNOWN`, budget remaining | stay `FIRED_UNCONFIRMED`, backoff | `:233-238`; budget at `src/core/intent_recovery.py:470-506` |
| `UNKNOWN`, budget exhausted | `PERMANENTLY_AMBIGUOUS` | `src/core/intent_recovery.py:478-487` |

A read-back that raises is swallowed to `observation = None`
(`src/core/intent_recovery.py:459-460`), which becomes `UNKNOWN` and therefore
...
(`src/core/intent_recovery.py:379-396`), which advances the version and
...
(`src/core/intent_recovery.py:455-458`). The transition table has no edge back
to `ABOUT_TO_FIRE` — Python set `src/core/intents.py:65-96`, re-checked inside
...
inside the script (`src/core/intents.py:573-599`), mirrored in Python
...
- **A1 — the recovery loop runs.** `run_forever`
  (`src/core/intent_recovery.py:263`) is a library coroutine; the repository
...
- **A2 — the loop survives its own pass. NOW ENFORCED.** Phase 1A recorded that
  ...
  per-execution discovery failures (`src/core/intent_recovery.py:213-221`),
...
- **A3 — the lease is eventually obtainable.** `_acquire` returns `None` after
  the configured attempts (`src/core/intent_recovery.py:321-333`); progress
...
- **R2-4 — classification inherits connector truthfulness.** `FIRED_CONFIRMED`
  ...
  (`src/core/intent_workflow.py:591-597`) with no independent verification.
- **R2-5 — the capability declaration is self-asserted.** The contract is now
  typed and validated for *shape* (`src/core/connector_contract.py:118-140`),
...
- **R2-7 — the authorization proves *a barrier ran in this process*, not that
  ...
  (`src/core/intents.py:649-675`), that script is reachable only through
  `authorize_dispatch`, and `authorize_dispatch` first consumes an
  unforgeable, single-use, scope-bound `DurabilityAck`
  (`src/core/intents.py:1110-1115`). Two gaps remain and are declared:
  ...
  2. a caller inside this process could compose `RequestBindingService.verify`
     ...
     (`src/core/request_binding.py:1489-1503`).
  The authorization key is also **not itself barriered** (`src/core/intents.py:673`
...
- **R2-8 — single-use is by attempt, not by consumption.** The preflight
  *verifies* the authorization but does not delete it
  (`src/core/intents.py:639`); it expires by TTL (`:673`). Replay is
...
(`src/core/intent_workflow.py:78-79`), both required positive (`:106-112`).
Backoff is full-jitter over `min(base·2^(n−1), cap)` with base 5 s and cap
300 s (`:80-81`, `:126-133`), sampled at
`src/core/intent_recovery.py:492-495`.

**Termination test.** `src/core/intent_recovery.py:478-483`; on exhaustion the
...
exactly one (`src/core/intents.py:561-563`), and the first entry must start at
...
(`src/core/intent_recovery.py:145-151`); execution status becomes `PAUSED`
(`src/core/intents.py:945-950`); no new intent may be created
(`src/core/intents.py:485`, `:495-497`); operator resolution is possible only
via `PERMANENTLY_AMBIGUOUS → FIRED_CONFIRMED`/`FAILED_CONFIRMED`
(`src/core/intents.py:87-94`; Lua `:458-459`), requiring the lease and exact
...
TTL (`src/core/intents.py:592-599`, constant `:49`). Runner and recovery refuse
to start unless the store TTL covers the maximum reconciliation duration plus a
7-day operator window (`src/core/intent_workflow.py:195-203`;
`src/core/intent_recovery.py:127-134`).
...
- **R3-3 — lease contention does not consume budget**
  (`src/core/intent_recovery.py:321-333`, `:363-364`).
...
- **R3-5 — escalated records still expire. CONFIRMED BY PROBE.** Phase 1B
  ...
  retention to seconds (`src/core/intents.py:592-599`; regression
...
- **R3-6 — recovery discovery is not memory-bounded.** `scan_once` still
  ...
  (`src/core/intent_recovery.py:202-236`); raised in
  `docs/07-phase2-gap-audit.md:33` and still true. The isolated-failure list is
  bounded (`src/core/intent_recovery.py:57`), the candidate list is not.
...
|---|---|---|---|
| P1 Fenced state | `src/core/intents.py:372-603` (`:385`, `:399-400`, `:601`); `src/core/storage.py:159-268` | Atomic, in-Redis | Bypass by any direct writer (R1-1); AOF rewind (R1-3, probe-confirmed) |
| P2 Detectable ambiguity | ordering `src/core/intent_workflow.py:424-574`; ack chain `src/core/durability.py:161-176` → `src/core/intents.py:1081-1162` → `:649-675` → `:639`; recovery `src/core/intent_recovery.py:403-521`; table `src/core/intents.py:449-461` | Atomic for the state machine and for the authorization check; **in-process** for the ack itself | Cannot prove fsync to Lua (R2-7); connector truthfulness (R2-4) |
| P3 Fail-closed bound | `src/core/intent_recovery.py:478-487`; budgets `src/core/intent_workflow.py:78-79` | Per-intent, conditional on progress | No escalation mechanism (R3-4); escalated records expire (R3-5, probe-confirmed) |
...
| NC-3 | **HA, consensus, or split-brain immunity.** | Single instance; the lock is a lease (`docs/01-hld.md:24-25`, `src/core/locks.py:9-13`). This is also why R1-3 has no local fix. |
...
| NC-5 | **Durability beyond one local AOF fsync.** | `WAITAOF 1 0` requires one *local* fsync (`src/core/durability.py:318`, `:332`); `WAIT` is explicitly not a substitute (`docs/06-phase2-design.md:228-231`). |
| NC-6 | **Recovery of an effect whose intent was lost with the host.** | `docs/06-phase2-design.md:292-294`. |
| NC-7 | **Authoritative negative inference from eventually consistent APIs.** | `NOT_APPLIED` is honoured only from `AUTHORITATIVE_READBACK` (`src/core/connector_contract.py:212-217`); positive-only absence is a named violation (`:218-224`). |
...
| NC-9 | **Operator alerting or incident delivery.** | Not implemented (§5.2). Recovery now records isolated scan failures and can call a `scan_failure_alert` (`src/core/intent_recovery.py:153-190`), but nothing alerts on `PERMANENTLY_AMBIGUOUS`. |
| NC-10 | **Unbounded ledger retention.** | TTL-based (R1-2, R3-5); `docs/06-phase2-design.md:409-410`. |
| NC-11 | **Production readiness.** | `PRODUCTION` mode fails closed: no production vault or connector exists (`src/core/intent_workflow.py:284-301`, `src/core/request_vault.py:1-6`). Measurement runs in `EVALUATION` mode, whose weaker properties are declared in §1.7. |
...
## 5. Enforcement gaps: designed but not enforced in `src/core/`
...
(`src/core/intent_workflow.py:591-597`) with no read-back cross-check. The
...
frozensets (`src/core/intent_workflow.py:84-90`).
...
first `FIRED_UNCONFIRMED`. `src/core/` emits neither. Phase 1B added
`scan_failure_alert` for *isolated scan failures* only
(`src/core/intent_recovery.py:124`, `:180-190`) and `recovery_lag_alert`
...
safe identifier (`src/core/intents.py:177-180`). Any caller holding the lease
and current version can drive `PERMANENTLY_AMBIGUOUS → FAILED_CONFIRMED` and
re-enable new attempts (`src/core/intents.py:507-509`). The design's audited
...
`risk_acceptance_id` (`src/core/intents.py:506`).
...
| Artifact | Role |
|---|---|
| `src/core/intents.py:372-603` | Intent CAS script — enforcement point for P1 and the state machine |
| `src/core/intents.py:608-641` | Pre-dispatch preflight, including the Phase 1B authorization check (`:639`) |
| `src/core/intents.py:649-675` | Dispatch-authorization script (Phase 1B) |
| `src/core/intents.py:1081-1162` | `authorize_dispatch`: consumes the ack, then writes the authorization |
| `src/core/intents.py:65-96`, `:449-461` | Exhaustive transition table, Python and Lua copies |
| `src/core/durability.py:62-176` | `dispatch_scope`, `DurabilityAck`, the provenance boundary, and the sole mint point |
| `src/core/durability.py:229-332` | WAITAOF capability validation and the fsync barrier |
| `src/core/connector_contract.py` | The production response-class contract and total classification |
| `src/core/intent_recovery.py:192-319` | Fault-isolated scan pass and surviving `run_forever` |
| `src/core/intent_recovery.py:347-521` | Claim-and-reconcile, typed-contract classification |
| `src/core/intent_workflow.py:39-52`, `:226-309` | `DispatchMode` and the composition gate |
| `src/core/request_vault.py:321-451` | The evaluation vault |
| `src/core/storage.py:159-268` | Phase 1 base CAS, including Phase 2 state protection |
| `src/core/locks.py:35-70`, `:119-240`, `:244-432` | Lease primitives and the capped auto-renewing lease |
