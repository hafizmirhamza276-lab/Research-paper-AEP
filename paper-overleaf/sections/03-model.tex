...
\begin{table}[t]
\centering
\caption{The six crash points, and what exists in the world at each
\emph{when the kill is delivered at the position named}. The right-hand column
is why the declared-ambiguity rate in \cref{sec:eval-rq1} varies so sharply
across them. This table describes the intended position. For the five baseline
systems the kill at \texttt{after\_barrier\_before\_dispatch} was delivered
later than the position named here, and \cref{sec:eval-setup} states what was
delivered instead.}
