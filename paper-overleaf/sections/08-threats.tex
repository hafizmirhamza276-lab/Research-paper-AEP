...
with a transactionally written ground-truth ledger, run at the delay, timeout
and error rates \cref{sec:eval-setup} states. It is realistic in the dimension
...
\paragraph{B4 is our model of a durable-execution engine, and no rate here is
the engine's} \Cref{sec:eval-rq1} reports the production engine run against the
same endpoint in both configurations B4 and B4b instantiate, together with the
four things that bound it. The threat this leaves is narrow: every B4 and B4b
rate in this paper is our implementation's, and no rate here should be read as
any engine's. We registered that obligation before collecting the engine data
rather than after seeing it. We do not compare the engine's rates against B4's,
for the reason \cref{sec:eval-rq1} gives.
