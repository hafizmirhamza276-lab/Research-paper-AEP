...
\item Write the intent \textsc{none}$\rightarrow$\textsc{about-to-fire} by
      ...
      pinned connection}.
\item Convert the barrier's acknowledgment into a Redis-visible dispatch
      authorization.
...
Two of these steps can refuse, and both refusals happen before any
...
may create version~1 again. An AOF rewind can also un-fence, and that is
confirmed by a probe rather than argued. P1 is a statement about one monotonic
Redis timeline, and lock keys are by design not barriered, so a lease
whose release was lost can reappear alongside a rewound version and a
previously fenced writer then satisfies both conjuncts. There is no local fix;
the fix is consensus, which is a non-claim (\cref{tab:nonclaims}).
...
later by a person who holds the lease and the exact version and asserts
...
\item Ordinary callers cannot construct, subclass or copy
      ...
      one attempt cannot authorize another through the supported path.
...
transmit. Nothing structural prevents that, because both principals are acting
on the same fenced state and a lease can expire at any moment. What prevents it
...
courtesy about late responses. It is also a real-time argument, which is why
...
reference to the mutation call at all and can only read back. The
...
Observation & Outcome \\
\midrule
capability undeclared / unrecognized & \textbf{ambiguous}, no query \\
...
direction} A crash after a durable intent but before transmission yields
...
the data: ambiguity is at its maximum at the two crash points where no effect
can exist.
...
and nothing alerts. Escalated records also expire: a retention floor of 31
...
the implementation. Solid edges are automated; the dashed edges
from \textsc{permanently-ambiguous} are the operator-only transitions. Every
state with a thick border is terminal for automated reconciliation. There is no
edge
...
transcribed from the implementation, meaning the Lua guards, the exhaustive
transition table, the creation fence and the recovery classification, and not
from this section. A gate re-derives the transition table and
...
The endpoint is modeled as \cref{sec:model-endpoint} describes it, and this is
the modeling decision the result rests on. It is never given an intent
...
and no protocol action may read it, which is enforced textually across the
...
\midrule
% Steps are COUNTEREXAMPLE LENGTHS -- the number of States in the trace TLC
% prints -- at the committed bounds; one row per failing file in
% formal/configs/. Not the DEPTH column scripts/run_tlc.sh prints, which is the
% search depth reached and is a larger number: aof-rewind returns a 4-state
% counterexample at search depth 5, with 24 states left on the queue. Lengths
% are used rather than state counts because a failing run stops at its first
% counterexample, so its state count reflects the search order and is not
% reproducible.
...
out of the model rather than modeled badly, because an uninterpreted function
...
acknowledgment chain of the four links above is \emph{assumed}, not checked:
what the model checks is the part that is atomic and Redis-visible, the
authorization key and the preflight that requires it. Real time is not
modeled. Where that cuts in the protocol's favor it is neutralized, since a
lease may expire at any instant, which is more adversarial than the budget;
where it cuts against, it is the reconciliation delay named above. Liveness is
checked within the version bound, so it establishes that nothing gets stuck for
a reason internal to the protocol, not termination in an unbounded run.
...
is. Two rows are worth reading twice. The AOF-rewind
residual of \cref{sec:p1}, previously argued and then confirmed by a probe,
falls out in four steps once the single-timeline assumption is dropped. The
...
second step would not appear. And the properties listed above as not checked
remain unchecked, most of the implementation's security argument among them.
What model checking changes is narrower than it looks and still worth having:
the composition of the fences, the state machine and the classification table
was previously an argument over code paths and is now a checked one, together
with the price of each assumption it rests on in \cref{tab:modelchecking}.
